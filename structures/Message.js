...
const format = (name, reverse) => {
    ...
    name.split(' ').forEach((s) => {
        final.push(`${s.charAt(0).toUpperCase()}${s.substr(1, s.length).toLowerCase()}`); 
    });
...
class Message {
    ...
    constructor(pageContent, username){
        ...
        if (!existsSync(`./data/${this.name}/lastMessage.json`))
            writeFileSync(`./data/${this.name}/lastMessage.json`, `{ "date": null }`, "utf-8");
            date = "000";
        ...
        this.isRead = !this.pageLastMessage.children[3].children[1].attribs.class.includes("b-like");
        ...
        this.formattedDateWithoutDay = this.formattedDate.split(' ').filter((i) => !dayNames.includes(i)).join(' ');
        this.date = dateAndTime.parse(this.formattedDateWithoutDay, "D MMMM YYYY à HH:mm");
    ...
    saveIt () {
        writeFileSync(`./data/${this.name}/lastMessage.json`, JSON.stringify({
