...
class Cours {
    ...
    constructor(coursData){
        ...
        this.matiere = formatMatiere(coursData.ListeContenus.V.find((v) => v.G === 16).L || 'Unknown');
        ...
        this.formattedStartDate = `${("0" + this.startDate.getHours()).slice(-2)}h${("0" + this.startDate.getMinutes()).slice(-2)}`;
        // La date de fin de cours
        this.endDate = new Date(this.startDate.getTime()+(coursData.duree*(60000*15)));
        this.formattedEndDate = `${("0" + this.endDate.getHours()).slice(-2)}h${("0" + this.endDate.getMinutes()).slice(-2)}`;
        // La salle du cours
        this.salle = (coursData.ListeContenus.V.find((v) => v.G === 17) || { L: 'Unknown' }).L;
        // Si le cours est annulé
        this.annule = coursData.estAnnule || false;
        // Si le cours est exceptionnel
        this.exceptionnel = (coursData.Statut && coursData.Statut === "Exceptionnel") || false;
        // Si le cours est modifié
        this.modifie = (coursData.Statut && coursData.Statut === "Cours modifi\u00E9") || false;
        // Si la salle est changée
        this.salleChangee = (coursData.Statut && coursData.Statut === "Changement de salle") || false;
        // Si le cours est déplacé
        this.deplace = (coursData.Statut && coursData.Statut === "Cours d\u00E9plac\u00E9") || false;
    }
